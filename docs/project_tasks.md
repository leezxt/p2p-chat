...
> 手機端一次性 QR 請求檢閱、公開金鑰 binding 與私鑰持有 proof gate 可先行，但 V3 實際桌面／
...
| P3 | Desktop Link、Device Sync、Revoke | V3 | ✅ 主機安全核心、手機端 QR 請求檢閱、公開金鑰 binding 與 RAM-only 私鑰持有 proof gate：明確授權、SQLite v13 同步切點、SQLite v14 一次性 request state、SQLite v15 public-key binding、雙向 X25519 `crypto_box` challenge-response、只選授權後新訊息、撤銷 fail-closed；桌面 UI／presentation、transport、per-device 加密與副端撤銷 runtime 驗收仍待完成 |
...
V3 的實際桌面端 UI／presentation、相機 QR runtime、目標主裝置交付、原生私鑰持有 proof
runtime、跨裝置協定與每副端金鑰生命週期仍需多端驗收；下列安全決策與手機端檢閱 UX 可先在
Windows host 建立並以 Dart／SQLite／Widget 與 test-only crypto fake 測試驗證。

| ID | 主機可完成範圍 | 主機驗收條件 | 仍需外部驗證 |
|---|---|---|---|
| V3-01 | Desktop Link／Device Sync／Revoke 安全核心（✅） | 獨立模組、SQLite v13 授權資料、主裝置拒絕成為副端、fingerprint 變更 fail-closed、只選 `createdAt > authorized_after` 的新訊息、撤銷後拒絕新選取、Event Bus 事件、v1→v13 migration 與 service tests 通過 | 桌面端 pairing presentation／連線、per-device 加密與金鑰銷毀、真實同步／網路故障、Windows／Android／iOS runtime、已撤銷副端無法解密新訊息的端對端證明 |
| V3-02 | 一次性 QR 配對請求與手機明確確認（✅） | SQLite v14 `desktop_link_pairing_requests`、嚴格 schema／目標主裝置／30–600 秒到期／一次性 request ID、`pending → confirming → confirmed/rejected`、掃描或貼入後預覽 fingerprint、拒絕不授權、明確同意後才呼叫 V3-01 | 真實 Android／iOS 相機權限與掃碼、桌面端 UI／交付、Desktop transport、per-device 加密、跨裝置資料同步與撤銷後新訊息無法解密的 runtime 證明 |
| V3-03 | QR 公開金鑰 binding 與 canonical request issuer（✅） | QR schema v2 強制 32-byte X25519 `publicKey`，手機端重算 device ID＋key fingerprint 並拒絕不一致內容；`DesktopLinkPairingRequestIssuer` 產生短效 canonical request；SQLite v15 作廢無法補齊 binding 的 v14 暫存 request | 桌面端 QR presentation、目標主裝置發現／交付、真實相機／桌面 runtime 與跨裝置同步驗收 |
| V3-04 | 私鑰持有 challenge-response 與手機授權 gate（✅） | 沿用既有 X25519 `MessageBox` 的雙向 authenticated `crypto_box`；短效 token 僅存 RAM，完整驗證 request／裝置／公開金鑰 fingerprint／challenge／token／效期；竄改、錯 key、重放、過期與更換 public key 皆 fail-closed，沒有有效 proof 時 `confirm` 拒絕授權；35 項 V3 專項 host tests 通過 | 桌面 responder／presentation、目標主裝置發現與自動交付、原生 libsodium／secure-storage proof runtime、真實相機、Desktop transport、per-device 加密、跨裝置同步與撤銷 runtime 驗收 |
