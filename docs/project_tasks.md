...
| ID | 任務 | 依賴 | 驗收條件 |
|---|---|---|---|
| V1-01 | 🚧 建立兩台真機的完整 E2E 測試腳本 | S8-05 | 雙 AVD 已分別通過 encrypted P2P runner，以及真實 sodium offline mailbox、ACK 遺失、App force-stop/restart、DELIVERED/READ 與 sender 本機 READ 狀態；Firebase Test Lab 單一實體機 instrumentation build/workflow 已實作，本機 app/test APK 封裝通過，待 GCP variables/真實 matrix；兩台真機、實際斷網與資源量測仍待驗 |
