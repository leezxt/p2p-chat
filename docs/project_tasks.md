...
| S7-02 | ✅ 實作密文上傳、拉取與刪除 backend | S7-01 | Flyway V4、JWT/device/contact ACL、冪等 upload、recipient pull/cursor、ACK、sender status、quota/rate limit、TTL cleanup 與 ciphertext 清除完成 |
...
| V1-02 | 安全與隱私稽核 | S6-06,S7-06,S8-05 | 私鑰/明文不上傳、不進 log；API 授權與重放防護驗證通過 |
| V1-03 | 資源目標量測 | V1-01 | 冷啟動、閒置記憶體、背景連線、網路與電量結果寫入 docs |
| V1-04 | 🚧 故障恢復與資料完整性測試 | V1-01 | SQLite 關閉／重開、ACK 遺失後重投、replay/receipt/message 冪等整合測試已通過；真機殺程序、斷網與 migration upgrade matrix 待驗 |
