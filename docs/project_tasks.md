...
> host presentation 與受限的 GitHub-hosted Windows module-route／App-entry runtime 可先行，但完整桌面／
...
| P3 | Desktop Link、Device Sync、Revoke | V3 | ✅ 主機安全核心、手機端 QR 請求檢閱、公開金鑰 binding、RAM-only 私鑰持有 proof gate、Desktop Companion host presentation 與受限 GitHub Windows module-route／App-entry runtime：明確授權、SQLite v13 同步切點、SQLite v14 一次性 request state、SQLite v15 public-key binding、雙向 X25519 `crypto_box` challenge-response、桌面 QR／手動 challenge responder、`/desktop-link` route 選擇 Companion、無後端 `bootstrap` → `P2pChatApp` → 聊天工具選單到 Companion、只選授權後新訊息、撤銷 fail-closed；完整 desktop runtime、transport、per-device 加密與副端撤銷 runtime 驗收仍待完成 |
...
| V2-02 | 內建貼圖與貼圖包工具（🚧 主機核心、單機 AVD UI 與雙 AVD Mailbox restart Gate 完成；外部 Gate 待驗） | 訊息只傳 sticker ID；manifest schema、大小／格式／授權檢查與 zip-slip 防護；內建資產與離線 schema tests；正式 `P2pChatApp` 的 Android AVD picker → `flutter` 選取 → PNG 渲染 → SQLite ID-only message；兩個 Android AVD 以真實 sodium／H2 Mailbox 完成 ACK-loss、force-stop、restart、冪等重拉與 ID-only sticker payload 驗證 | 完整 `MessageTransportCoordinator` P2P→Mailbox fallback 編排、兩台 Android 真機的離線同步／重送、真機資源占用與大量貼圖包操作 |
...
CI 另可驗證受限的 native sodium pairing flow、`DesktopLinkModule` 的 module-route 選擇，以及無後端
`bootstrap` → `P2pChatApp` → 聊天工具選單的 App-entry 路由，但不取代真實手機或跨裝置 runtime。
...
| V3-05 | Desktop Companion QR／手動 challenge presentation（✅） | desktop target route 顯示手機 ID、名稱輸入、canonical QR、encrypted challenge 貼入與 encrypted response 複製；無網路、無背景、無 SQLite token／訊息 state；service／Widget 與既有 V3 共 38 項 host tests 通過。[GitHub Windows CI run 31319510492](https://github.com/leezxt/p2p-chat/actions/runs/31319510492) 另通過真實 `SodiumMessageBox`、QR rendering、主端角色 proof、受控 clipboard copy，以及 SQLite FFI／`ModuleRegistry`／`RouteRegistry`／`Navigator` test shell 到 Companion 的 module-route 驗證；新增 integration 以無後端正式 `bootstrap`、`P2pChatApp` 與聊天工具選單走到 Companion；`tool/verify_windows_desktop.ps1` 拒絕以 `NIX_SKIP_SODIUM_BUILD_HOOKS` 作 native evidence | macOS／Linux runner、目前本機 Windows 已安裝桌面 App 的人工使用者驗收、真實手機相機、目標主裝置 discovery／自動交付、Desktop transport、per-device 加密、跨裝置同步與撤銷 runtime 驗收 |
