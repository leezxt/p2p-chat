...
GitHub-hosted Windows CI 的 [run 31318234695](https://github.com/leezxt/p2p-chat/actions/runs/31318234695)
...
`/desktop-link` 選擇 Companion。既有同一 Windows job 也通過 debug bundle、native crypto、Credential
Manager 跨 process 與 WebRTC integration。這是受限的 Windows runtime 證據；本機仍缺 Visual Studio
C++ workload，且未驗收完整 `bootstrap`／桌面使用者流程、實體手機相機／QR、目標主裝置自動交付、
...
- **V3** 多裝置 / 進階：Desktop Link 主機安全核心、一次性 QR 請求檢閱／明確確認、
  ...
  GitHub-hosted Windows native Companion／module-route integration 已完成；完整 Desktop 使用者流程／自動交付、
