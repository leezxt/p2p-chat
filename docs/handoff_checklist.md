...
- [ ] **已實作未驗證**：V1 CI 三個 checkout steps 與一個 setup-java step 已更新至上述完整 commit SHA；`contents: read`、Ubuntu 24.04、timeouts、Java/Maven cache 與 Flutter pin 均未改動
- [x] **已完成**：本機確認所有 5 個外部 actions 都使用完整 40 字元 SHA、舊 SHA 已移除，`git diff --check` 通過；本機沒有 `actionlint`，YAML/runtime 由 PR CI 驗證
- [ ] **未完成**：提交、PR 三項 V1 CI、確認 Node.js 20 annotation 消失、合併與 `main` CI

Runtime：本輪沒有啟動 Docker、ADB、AVD、App 或 backend；只讀取 GitHub 官方 release、
tag、action metadata 與 verified commit 資訊。

下一步：推送 `codex/ci-node24-actions` 並等待 Java、Flutter、PostgreSQL container smoke；
三個 jobs 都不得再出現 Node.js 20 deprecation annotation，通過後才勾選完成並合併。
