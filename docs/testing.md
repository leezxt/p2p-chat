...
| Windows Desktop | `flutter test --no-pub test/modules/device_key_service_test.dart` | `tool/verify_windows_desktop.ps1 -Build -Runtime`；CI 再加跑 Desktop Companion 與 Desktop Link module-route runtime integrations | libsodium native runtime、Credential Manager、crypto failure paths、WebRTC DataChannel、Desktop Companion QR／challenge flow、ModuleRegistry／RouteRegistry／Navigator route selection、Runner 與 plugins 編譯 |
...
`Navigator` test shell 驗證 Windows target 的 `/desktop-link` 會選擇 Companion。若要在本機額外測
copy，需明確加上 `-VerifyClipboard`，因為它會寫入並清除測試 clipboard。CI 在 GitHub-hosted
Windows runner 會加上這個旗標。腳本會拒絕設定 `NIX_SKIP_SODIUM_BUILD_HOOKS` 的 native run，
避免 fake crypto 成為 runtime evidence。

這些測試證明 Windows runner 可載入相關 plugin，並驗證 desktop pairing flow 與受限的模組路由；
[PR #54 V1 CI run 31318234695](https://github.com/leezxt/p2p-chat/actions/runs/31318234695) 已在
`59fcad1` 對應版本通過 Companion 與 module-route integrations。route test 不啟動完整
`bootstrap` 或首頁使用者旅程；這些測試也不啟動真實手機相機、不建立 Desktop transport、
per-device re-encryption、跨裝置訊息同步或撤銷後資料不可解密的端對端情境。
